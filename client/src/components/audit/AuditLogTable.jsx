...
import { FileText, Eye, ShieldCheck, AlertCircle, Loader2 } from 'lucide-react';
          ...
          <table className="w-full text-left border-collapse text-xs relative">
            ...
            <tbody className="divide-y divide-neutral-200/70 dark:divide-neutral-800/80 font-mono">
              {auditLogs.length === 0 ? (
                ...
                auditLogs.map((log) => {
                  const financialVal = log.financialImpact > 0
                    ? log.financialImpact
                    : (log.payload?.initialRevenueAtRisk || log.payload?.amount || log.payload?.recoveredAmount || 0);
                    ...
                    <tr key={log.auditId} className="hover:bg-neutral-50/80 dark:hover:bg-neutral-900/50 transition-colors">
                      ...
                      <td className="py-2.5 px-3.5 text-right font-semibold text-neutral-900 dark:text-neutral-100 whitespace-nowrap">
                        {log.financialImpact > 0 ? (
                          ...
                          <span className="text-neutral-700 dark:text-neutral-300">{formatINR(financialVal)}</span>
                        ) : (
                          <span className="text-neutral-500 dark:text-neutral-400 font-mono">{formatINR(0)}</span>
                        )}
                      </td>
                      <td className="py-2.5 px-3.5 text-center">
                        <button
                          ...
                          className="p-1 rounded text-neutral-500 hover:text-neutral-900 dark:hover:text-neutral-100 hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors cursor-pointer"
